...
def main_func():
    my_file = open("/network/scripts/690/av_input.txt", "r")
    ...
    for x in OutPut['offline']:
        socketlist_offline_email.append({x: "av device offline", })

    for x in OutPut['offline']:
        socketlist_offline.append(
            {'ip': x, 'Status': 'offline', '@timestamp': now, 'tag': 0})
    for x in OutPut['online']:
        socketlist_online.append(
            {'ip': x, 'Status': 'online', '@timestamp': now, 'tag': 1})
    ...
    keys1 = list2[0].keys()
    with open('/network/scripts/690/av_output.txt', 'wb') as output_file1:
    ...
    if OutPut['offline']:
        ...
        SUBJECT = "Ping failed for following 690 AV devices"
        ...
        TEXT = """\
    			<html>
    			<body>
    			<p>This is an automated email for ICMP/Ping failed 690 AV devices
    			    <br>
    			    <br>
    			    You are receiving this email alert as av_ping.py script has been executed
    			    <br>
    				<br>
