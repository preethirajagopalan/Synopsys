	...
	with open(LOG_FILE, 'w') as f:
		...
		for row in sorted(circuit_info, key = lambda entry : entry[2]):
			...
			if site != last_site:
				...
				# set new site
				last_site = site
				#print( "site is", site,"last site is", last_site,service)
			...
			for i in range(num_entries):
				...
				sql_circuit[i][2] = datetime.datetime.strptime((str(sql_circuit[i][2].split(".")[0])), '%Y-%m-%d %H:%M:%S')
				# if site == "AM04":
				# 	print site, service
				# 	print query
				# 	print sql_circuit
				#break
				#print(site, service, inbps, outbps, sql_circuit[i][2], bandwidth[0])
