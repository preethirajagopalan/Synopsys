...
'Netlab' and SITE != 'Solarwinds-US' and SITE != 'US';""")
    ...
    network_device_type_field = {'network_device_type': "site_count_all"}
...
def dc_site_list_details():
    ...
    cursor = conn_ussql.cursor()
    cursor.execute(
        """select DISTINCT SITE as items from [dbo].[Nodes] where Network_Device_Type = 'SWITCH_DC' or Network_Device_Type = 'PAN_DC_FW' AND SITE is not null and SITE != 'International' and SITE != 'Solarwinds-INTL' and SITE !=
'Netlab' and SITE != 'Solarwinds-US' and SITE != 'US';""")
    device_us = cursor.fetchall()
    ...
    cursor_int = conn_intsql.cursor()
    cursor_int.execute(
        """select DISTINCT SITE as items from [NetPerfMonInt].[dbo].[Nodes] where Network_Device_Type = 'SWITCH_DC' or Network_Device_Type = 'PAN_DC_FW' AND SITE is not null and SITE != 'International' and SITE != 'Solarwinds-INTL' and SITE !=
'Netlab' and SITE != 'Solarwinds-US' and SITE != 'US';""")
    device_int = cursor_int.fetchall()
    ...
    result1_site = ast.literal_eval(json.dumps(result))
    time_field = {'@timestamp': now}
    type_field = {'type': "interactive_dc_site_list"}
    network_device_type_field = {'network_device_type': "interactive_dc_site"}
    for n in result1_site:
        n.update(time_field)
        n.update(type_field)
        n.update(network_device_type_field)
    print("--------------------------------------------site_result----------------------------------------")
    keys = result1_site[0].keys()

    with open('/network/scripts/inventory/Result/interactive_dc_site_list.csv', 'wb') as csv_file:
        dict_writer = csv.DictWriter(csv_file, keys)
        dict_writer.writeheader()
        dict_writer.writerows(result1_site)
    return result1_site

def scs_site_list_details():
    password = get_password()
    # connection to us-SQL
    conn_ussql = pyodbc.connect('DRIVER=/usr/lib64/libtdsodbc.so.0;'
                                'SERVER=10.200.17.64;PORT=1433;UID=SolarWindsOrionDatabaseUser;PWD=%s' % password)
    cursor = conn_ussql.cursor()
    cursor.execute(
        """select distinct Site as items from [dbo].[Nodes] where Caption like '%sc9%' AND SITE is not null and SITE != 'International' and SITE != 'Solarwinds-INTL' and SITE !=
'Netlab' and SITE != 'Solarwinds-US' and SITE != 'US';""")
    device_us = cursor.fetchall()
    ...
    cursor_int = conn_intsql.cursor()
    cursor_int.execute(
        """select distinct Site as items from [NetPerfMonInt].[dbo].[Nodes] where Caption like '%scs%' AND SITE is not null and SITE != 'International' and SITE != 'Solarwinds-INTL' and SITE !=
'Netlab' and SITE != 'Solarwinds-US' and SITE != 'US';""")
    device_int = cursor_int.fetchall()
    ...
    result2_site = ast.literal_eval(json.dumps(result))
    time_field = {'@timestamp': now}
    type_field = {'type': "interactive_scs_site_list"}
    network_device_type_field = {'network_device_type': "interactive_scs_site"}
    for n in result2_site:
        n.update(time_field)
        n.update(type_field)
        n.update(network_device_type_field)
    print("--------------------------------------------site_result----------------------------------------")
    keys = result2_site[0].keys()

    with open('/network/scripts/inventory/Result/interactive_scs_site_list.csv', 'wb') as csv_file:
        dict_writer = csv.DictWriter(csv_file, keys)
        dict_writer.writeheader()
        dict_writer.writerows(result2_site)
    return result2_site

def dmz_site_list_details():
    password = get_password()
    # connection to us-SQL
    conn_ussql = pyodbc.connect('DRIVER=/usr/lib64/libtdsodbc.so.0;'
                                'SERVER=10.200.17.64;PORT=1433;UID=SolarWindsOrionDatabaseUser;PWD=%s' % password)
    cursor = conn_ussql.cursor()
    cursor.execute(
        """select distinct SITE as items from [dbo].[Nodes] where Network_Device_Type = 'SWITCH_DMZ' AND SITE is not null and SITE != 'International' and SITE != 'Solarwinds-INTL' and SITE !=
'Netlab' and SITE != 'Solarwinds-US' and SITE != 'US'""")
    device_us = cursor.fetchall()
    ...
    cursor_int = conn_intsql.cursor()
    cursor_int.execute(
        """select distinct SITE as items from [NetPerfMonInt].[dbo].[Nodes] where Network_Device_Type = 'SWITCH_DMZ' AND SITE is not null and SITE != 'International' and SITE != 'Solarwinds-INTL' and SITE !=
'Netlab' and SITE != 'Solarwinds-US' and SITE != 'US';""")
    device_int = cursor_int.fetchall()
    ...
    result3_site = ast.literal_eval(json.dumps(result))
    time_field = {'@timestamp': now}
    type_field = {'type': "interactive_dmz_site_list"}
    network_device_type_field = {'network_device_type': "interactive_dmz_site"}
    for n in result3_site:
        n.update(time_field)
        n.update(type_field)
        n.update(network_device_type_field)
    print("--------------------------------------------site_result----------------------------------------")
    keys = result3_site[0].keys()

    with open('/network/scripts/inventory/Result/interactive_dmz_site_list.csv', 'wb') as csv_file:
        dict_writer = csv.DictWriter(csv_file, keys)
        dict_writer.writeheader()
        dict_writer.writerows(result3_site)
    return result3_site
...
if __name__ == "__main__":
    ...
    dc_site_list_details = dc_site_list_details()
    scs_site_list_details = scs_site_list_details()
    dmz_site_list_details = dmz_site_list_details()
