...
try:
    ...
    a.Out_Maxbps, a.In_Minbps, a.Out_Minbps, a.TimeStamp, b.InterfaceName, b.InPercentUtil, b.OutPercentUtil,
    ...
    FROM [NetPerfMonInt].[dbo].[InterfaceTraffic_CS_Detail] as a WITH (NOLOCK)
    INNER JOIN [NetPerfMonInt].[dbo].[Interfaces] as b WITH (NOLOCK) 
    ON a.NodeID = b.NodeID
    AND a.InterfaceID = b.InterfaceID
    INNER JOIN [NetPerfMonInt].[dbo].[Nodes] as c WITH (NOLOCK) 
    ON a.NodeID = c.NodeID
    WHERE b.Status = 1 AND a.TimeStamp >= DATEADD(mi, -70, GETDATE()) AND c.Status = 1
...
if result:
    ...
    if count <= limit:
        ...
        msg['From'] = me
        msg['To'] = ", ".join(you)
        text = "No. of Entries from Intl-SQL (Interface)<= 1000"
        part1 = MIMEText(text, 'plain')
        msg.attach(part1)
    ...
    for item in items:
        ...
        if item['timestamp']:
            #date_time = datetime_to_utc(item['timestamp'], code='intl')
            item['timestamp'] = str(date_time)
        ...
        try:
            ...
            msg['From'] = me
            msg['To'] = ", ".join(you)
            text = "Connection to ens-db failed when checking with dnsname (Interface-Intl)" + "\n" + "Error: " + repr(ep)
            part1 = MIMEText(text, 'plain')
            msg.attach(part1)
            try:
                s = smtplib.SMTP('mailhost')
        ...
        if len(ls_data1) != 0:
            ...
            try: 
                ...
                msg['From'] = me
                msg['To'] = ", ".join(you)
                text = "Connection to ens-db failed when checking with assetname (Interface-Intl)" + "\n" + "Error: " + repr(ep)
                part1 = MIMEText(text, 'plain')
                msg.attach(part1)
                try:
                    s = smtplib.SMTP('mailhost')
...
# check if the index name exists, if yes then proceed with bulk upload, else create index before bulk upload
    if not es.indices.exists(index_name):
        ...
        try:
            ...
            msg['From'] = me
            msg['To'] = ", ".join(you)
            text = "Creating Index on Elasticsearch Failed (Interface-Intl)" + "\n" + "Error: " + repr(et)
            part1 = MIMEText(text, 'plain')
            msg.attach(part1)
            try:
                s = smtplib.SMTP('mailhost')
    ...
    msg['From'] = me
    msg['To'] = ", ".join(you)
    text = "No Result from SolarWinds-Intl-SQL" + "\n" + "Error: "
    part1 = MIMEText(text, 'plain')
    msg.attach(part1)
    try:
        s = smtplib.SMTP('mailhost')
