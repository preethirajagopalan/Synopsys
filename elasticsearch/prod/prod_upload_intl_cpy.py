...
es = Elasticsearch(hosts=[{'host': 'snpstemp054', 'port': 9200}], timeout=200)
...
try:
    ...
    a.Out_Maxbps, a.In_Minbps, a.Out_Minbps, a.DateTime, b.InterfaceName, b.InPercentUtil, b.OutPercentUtil,
...
if result:
    ...
    for item in items:
        ...
        if item['datetime']:
            ...
            if not "." in data_dict['datetime']:
                data_dict['datetime'] = data_dict['datetime'] + ".000000"
            dt = data_dict['datetime']

        # check for duplicates on ES
        if es.indices.exists(index_name):
            if dt and data_dict['interface.interfaceid']:
                intid = data_dict['interface.interfaceid']
                query = {
                    "query": {
                            "bool": {
                                "must": [{"match": {"interface.interfaceid": intid}},
                                        {"match": {"datetime": dt}}]
                                    }
                            }
                    }
                ...
                for r in res["hits"]["hits"]:
                    ch = r["_source"]
                    if ch["interface.interfaceid"] == intid and ch["datetime"] == data_dict["datetime"]:
                        data_dict['interface.interfaceid'] = "clear"
        ...
        try:
            ...
            msg['From'] = me
            msg['To'] = ", ".join(you)
            text = "Connection to ens-db failed when checking with dnsname" + "\n" + "Error: " + repr(ep)
            part1 = MIMEText(text, 'plain')
            msg.attach(part1)
            try:
                s = smtplib.SMTP('mailhost')
        ...
        if len(ls_data1) != 0:
            ...
            try: 
                ...
                msg['From'] = me
                msg['To'] = ", ".join(you)
                text = "Connection to ens-db failed when checking with assetname" + "\n" + "Error: " + repr(ep)
                part1 = MIMEText(text, 'plain')
                msg.attach(part1)
                try:
                    s = smtplib.SMTP('mailhost')
        ...
        # append data to above structure
        if not data_dict['interface.interfaceid'] == "clear":
