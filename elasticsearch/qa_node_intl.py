...
# Start of Data from SolarWinds-Int-SQL
...
try:
    ...
    cursor = conn_intsql.cursor()
    cur_1 = conn_intsql.cursor()
    ...
    cursor.execute("""SELECT a.Caption, a.Vendor, a.IOSVersion, a.MachineType,
                        ...
                        FROM [NetPerfMonInt].[dbo].[Nodes] as a WITH (NOLOCK)
                        INNER JOIN [NetPerfMonInt].[dbo].[HWH_HardwareInfo] as b WITH (NOLOCK) 
                        ON a.NodeID = b.ID
                        INNER JOIN [NetPerfMonInt].[dbo].[APM_HardwareSensorAlertData] as c WITH (NOLOCK) 
                        ON a.NodeID = c.NodeID
                        WHERE c.CategoryName = 'Temperature' and c.SensorDisplayName like '%s'
                        """ % ('Fex'+'%'))

    # gather all data from Solarwinds-Int-SQL
...
# catch the exception and send the error as-is
    ...
    msg['From'] = me
    msg['To'] = ", ".join(you)
    text = "Connection to Int-Sql Database Failed (Node)" + "\n" + "Error: " + repr(e)
...
if result or res_1:
    ...
    if count <= limit:
        ...
        msg['From'] = me
        msg['To'] = ", ".join(you)
        text = "No. Of Entries from Int-SQL <=100 (Node)"
        part1 = MIMEText(text, 'plain')
        msg.attach(part1)
    ...
    print('Bulk Data for Int created (Node)')
    print(now)

    # settings and mappings for solarwinds-int-sql data (found in upload_functions.py file)
    ...
    if not es.indices.exists(index_name):
        ...
        try:
            ...
            msg['From'] = me
            msg['To'] = ", ".join(you)
            text = "Creating Index on Elasticsearch Failed (Node - Intl)" + "\n" + "Error: " + repr(et)
            part1 = MIMEText(text, 'plain')
            msg.attach(part1)
            try:
                s = smtplib.SMTP('mailhost')
    ...
    try:
        ...
        print("Upload onto ES-Int Done")
    except Exception as et:
            msg = MIMEMultipart('alternative')
            msg['Subject'] = 'Error Report from Elastic Search'
            msg['From'] = me
            msg['To'] = ", ".join(you)
            text = "Inserting on Elasticsearch Failed (Node - Intl)" + "\n" + "Error: " + repr(et)
            part1 = MIMEText(text, 'plain')
            msg.attach(part1)
            try:
                s = smtplib.SMTP('mailhost')
    ...
    # set the messages for log (can be found in qa_node_intl_log.log file)
    print('Bulk Data for Intl-Node created')
    print(now)
    ...
    try:
        outcome = es.bulk(index=index_name, body=bulk_data)
        print("Upload onto ES-Intl node done")
    except Exception as et:
            msg = MIMEMultipart('alternative')
            msg['Subject'] = 'Error Report from Elastic Search'
            msg['From'] = me
    ...
    msg['From'] = me
    msg['To'] = ", ".join(you)
    text = "No Result from SolarWinds-Int-SQL (Node)" + "\n" + "Error: "
    part1 = MIMEText(text, 'plain')
    msg.attach(part1)
    try:
        s = smtplib.SMTP('mailhost')
...
# End of Data from SolarWinds-Int-SQL
